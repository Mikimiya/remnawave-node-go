...
func TestJWTMiddleware_ValidToken(t *testing.T) {
	...
	router := gin.New()
	router.Use(middleware.JWTMiddleware(publicKeyPEM, log))
...
func TestJWTMiddleware_WrongSigningKey(t *testing.T) {
	...
	var handlerCalled atomic.Bool
	router := gin.New()
	router.Use(middleware.JWTMiddleware(publicKeyPEM2, log))
	router.GET("/test", func(c *gin.Context) {
		handlerCalled.Store(true)
		c.Status(http.StatusOK)
	})
...
func TestJWTMiddleware_InvalidPublicKey(t *testing.T) {
	...
	var handlerCalled atomic.Bool
	router := gin.New()
	router.Use(middleware.JWTMiddleware("invalid-key", log))
	router.GET("/test", func(c *gin.Context) {
		handlerCalled.Store(true)
		c.Status(http.StatusOK)
	})
...
func TestJWTMiddleware_NilLogger(t *testing.T) {
	...
	var handlerCalled atomic.Bool
	router := gin.New()
	router.Use(middleware.JWTMiddleware(publicKeyPEM, nil))
	router.GET("/test", func(c *gin.Context) {
		handlerCalled.Store(true)
		c.Status(http.StatusOK)
	})
...
func TestParseRSAPublicKey_PKIX(t *testing.T) {
	...
	key, err := middleware.ParseRSAPublicKey(string(publicKeyPEM))
	if err != nil {
...
func TestParseRSAPublicKey_InvalidPEM(t *testing.T) {
	_, err := middleware.ParseRSAPublicKey("not a pem")
	if err == nil {
...
func TestParseRSAPublicKey_InvalidKey(t *testing.T) {
	...
	_, err := middleware.ParseRSAPublicKey(string(invalidPEM))
	if err == nil {
